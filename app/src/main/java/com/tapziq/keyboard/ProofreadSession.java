...
/** In-process handoff between the IME and the foreground proofreading activity. */
final class ProofreadSession {
    ...
    private static final ScheduledExecutorService EXPIRY_EXECUTOR =
            Executors.newSingleThreadScheduledExecutor(new ThreadFactory() {
                ...
                public Thread newThread(Runnable runnable) {
                    Thread thread = new Thread(runnable, "tapziq-proofread-expiry");
    ...
    /** Claims a launched bridge. Claimed text remains only for that activity's lifetime. */
    ...
    static synchronized Result peekResult(int id) {
        if (result == null || result.id != id || activeId != id) {
            return null;
        }
        return result;
    }
    ...
    static synchronized Result peekDeliverableResult(int id) {
        return deliveryReady ? peekResult(id) : null;
    }

    static synchronized Result takeResult(int id) {
        if (result == null || result.id != id || activeId != id) {
            return null;
        }
        Result value = result;
    ...
    static synchronized Result takeDeliverableResult(int id) {
        return deliveryReady ? takeResult(id) : null;
    }
    ...
    /** Signals only after the bridge has stopped, so private output is never shown behind it. */
    static void notifyResultReady(int id) {
        Listener listenerValue;
        synchronized (ProofreadSession.class) {
            if (!hasResult(id)) {
                return;
            }
            deliveryReady = true;
