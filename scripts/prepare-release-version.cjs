...
const SOURCE_VERSION_NAME_PATTERN =
  /^val tapziqSourceVersionName = "([^"\r\n]+)"$/gm;
const SOURCE_VERSION_CODE_PATTERN =
  /^val tapziqSourceVersionCode = ([^\s\r\n]+)$/gm;
...
function exactlyOneMatch(contents, pattern, description) {
  ...
  if (matches.length !== 1) {
    fail(`Tapziq source must contain exactly one ${description}.`);
  }
  return matches[0];
}
...
function sourceMetadata(contents) {
  const version = exactlyOneMatch(
    ...
    "tapziqSourceVersionName declaration",
  )[1];
  const rawVersionCode = exactlyOneMatch(
    contents,
    SOURCE_VERSION_CODE_PATTERN,
    "tapziqSourceVersionCode declaration",
  )[1];
...
function sourceWithVersion(contents, version) {
  ...
  const updated = contents
    .replace(
      SOURCE_VERSION_NAME_PATTERN,
      `val tapziqSourceVersionName = "${version}"`,
    )
    .replace(
      SOURCE_VERSION_CODE_PATTERN,
      `val tapziqSourceVersionCode = ${versionCode}`,
    );
...
function verifySourceVersion(version, {
  ...
  if (metadata.version !== version || metadata.versionCode !== expectedVersionCode) {
    fail(
      `Tapziq source is ${metadata.version} (${metadata.versionCode}), expected `
...
function main(arguments_) {
  ...
  if (command === "prepare" && version && third && extra.length === 0) {
    ...
    process.stdout.write(
      `${action} Tapziq source version ${result.version} `
        + `(${result.versionCode}).\n`,
    );
    return;
  }
  if (
    ...
    process.stdout.write(
      `Verified Tapziq source version ${metadata.version} `
