...
trusted_repository_id=1332440403
...
[[ "$repository" =~ ^[^/]+/[^/]+$ ]] || \
  fail "Could not resolve the trusted Tapziq repository."

verification_directory="$(
  mktemp -d "${RUNNER_TEMP:-${TMPDIR:-/tmp}}/tapziq-published.XXXXXX"
)"
...
cleanup() {
  rm -f \
    "$release_json" \
    "$download_directory/Tapziq-v$release_version.apk" \
...
if ! jq -e \
    --arg tag "$release_tag" \
    --arg title "Tapziq Keyboard $release_version" \
    --arg apk "$apk_name" \
    '
      .tag_name == $tag
      and .name == $title
